...
class Builder:
  def __init__(self):
    ...
    self.flags = f"-O2 -std=c++20 -I{self.include} -Wno-switch"
    self.linker_flags = f"-Wl,--gc-sections -mwindows"
  ...
  def run(self, with_multi_thread: bool):
    ...
    if self.updated or not os.path.exists(self.output):
      print("linking...")
      os.system(f"{self.compiler} {self.linker_flags} {' '.join(self.objects)} -o {self.output}")
    else:
